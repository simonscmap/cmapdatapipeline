...
test_fil = test_dir+'PiscesTest20230208.nc'
txd =  xr.open_dataset(test_fil)
test_data_df = txd.to_dataframe().reset_index()
txd.close()
test_data_cols = test_data_df.columns.tolist()
...
if test_df_hash !=test_data_hash:
    ...
    for c in test_df.columns.tolist():
        if pd.util.hash_pandas_object(test_data_df[c]).sum() != pd.util.hash_pandas_object(test_df[c]).sum():
            print(f"Different values in {c}")
...
test_fil = nrt_dir+'tblPisces_Forecast_cl1_2020_11_01.parquet'
...
def proc_pic(fil):
    ...
    s3_str = f"aws s3 cp {tbl}_{fil_date}.parquet s3://cmap-vault/model/tblPisces_Forecast_cl1/nrt/"
    os.system(s3_str)
