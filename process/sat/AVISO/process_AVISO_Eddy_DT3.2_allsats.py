...
SQL.full_SQL_suggestion_build(df, 'tblMesoscale_Eddy_allsats_untracked', 'satellite', 'Rainier', 'Opedia')
...
for fil in tqdm(flist_all):
    ...
    if '_long' in fil:
        ...
        DB.toSQLbcp_wrapper(df, 'tblMesoscale_Eddy_allsats_short', 'Rainier')
    elif '_untracked' in fil:
        DB.toSQLbcp_wrapper(df, 'tblMesoscale_Eddy_allsats_untracked', 'Rainier')

    #Long Trajectories (lifetime >= 10 days)

tbl_list = ['tblMesoscale_Eddy_allsats_long', 'tblMesoscale_Eddy_allsats_short', 'tblMesoscale_Eddy_allsats_untracked']
