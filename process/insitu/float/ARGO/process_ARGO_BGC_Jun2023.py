...
argo_core_path = vs.float_dir + f'tblArgoCore_REP_{date_string}/raw/'
argo_rep_path = f'{vs.float_dir}{tbl}/rep/'
meta_path = f'{vs.float_dir}{tbl}/metadata/'


flist = glob.glob(argo_core_path+'*.tar.gz')
...
# fil = '/data/CMAP Data Submission Dropbox/Simons CMAP/vault/observation/in-situ/float/tblArgoBGC_REP_Apr2023/raw/ARGO/202304-ArgoData/dac/BGC_subset/4902437_Sprof.nc'
# # New column needs to be added: PROFILE_DOWN_IRRADIANCE665_QC
# # 4902437_Sprof
# ## empty: 5906555, 2901705, 5903660, 4902521
# # fil = vs.float_dir+'/tblArgoBGC_REP_Apr2023/rep/tblArgoBGC_REP_Apr2023_4902521_Sprof.parquet'
...
metadata.export_script_to_vault(tbl,'float_dir','process/insitu/float/ARGO/process_Argo_BGC_June2023.py','process.txt')

# for fil in tqdm(BGC_flist):
#     xdf = xr.open_dataset(fil, mask_and_scale=True)
#     xdf = cmn.decode_xarray_bytes(xdf)  
#     if 'TEMP_CNDC' in list(xdf.keys()):
#         print(xdf.TEMP_CNDC)
#         print(xdf.TEMP_CNDC_ADJUSTED)
#     if 'CHLA_FLUORESCENCE' in list(xdf.keys()):
#         print(xdf.CHLA_FLUORESCENCE)
#         print(xdf.CHLA_FLUORESCENCE_ADJUSTED)
#     xdf.close()        
# flist = glob.glob(argo_rep_path + "*.parquet")
# for f in tqdm(flist):
