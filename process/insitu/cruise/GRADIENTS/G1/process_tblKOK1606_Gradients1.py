...
df_join = df_join[['time', 'lat', 'lon', 'depth', 'stnnbr', 'castno', 'rosette', 'ctdprs', 'ctdtmp',
       'ctdsal', 'ctdoxy', 'ctdchl', 'theta', 'sigma', 'oxygen', 'chla', 'ctdtmp_flag', 'ctdsal_flag',
       'ctdoxy_flag', 'ctdchl_flag', 'theta_flag', 'sigma_flag', 'oxygen_flag',
       'chla_flag']]

df_join['lon'] = df_join['lon'].astype(np.float) * -1
df_join.to_excel(raw_folder+'tblKOK1606_Gradients1_data.xlsx', index = False)
df_join.to_csv('tblKOK1606_Gradients1_data.csv', index = False)
