...
Script to process raw BATS Bottle Validation data
"""
...
df = pd.read_csv(flist[0], skiprows=57)
...
df = pd.read_csv(flist[0], sep='\t', skiprows=58, names=renamed)
...
## Drop empty columns so validator accepts submission
df = data.remove_blank_columns(df)

if export_data:
    df.to_excel(base_folder+f'/{tbl}_data.xlsx', index=False)
    metadata.export_script_to_vault(tbl,'cruise','process/insitu/cruise/BATS/process_BATS_Bottle_validation.py','process.txt')
