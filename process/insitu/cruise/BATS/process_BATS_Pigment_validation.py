...
Script to process raw BATS Pigment validation data
"""
import sys, os
import glob
import pandas as pd
...
tbl = 'tblBATS_Pigment_Validation'
...
df = pd.read_csv(flist[0], skiprows=48)
...
df = pd.read_csv(flist[0], sep='\t', skiprows=49, names=renamed)
...
if export_data:
    ...
    metadata.export_script_to_vault(tbl,'cruise','process/insitu/cruise/BATS/process_BATS_Pigment_validation.py','process.txt')
