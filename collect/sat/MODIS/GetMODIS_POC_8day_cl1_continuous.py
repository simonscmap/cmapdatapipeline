...
#https://oceandata.sci.gsfc.nasa.gov/cgi/getfile/AQUA_MODIS.20020704_20020711.L3m.8D.POC.poc.9km.nc
...
def get_POC(start_date,retry=False):
    ...
    file_url = f"https://oceandata.sci.gsfc.nasa.gov/cgi/getfile/AQUA_MODIS.{start_index}_{end_index}.L3m.8D.POC.poc.9km.nc"
    Original_Name = f"AQUA_MODIS.{start_index}_{end_index}.L3m.8D.POC.poc.9km.nc"
...
end_date = api.minDateCluster('tblModis_POC_NRT')
...
# df = api.query("select time, count(*) cnt from tblModis_POC_nrt group by time")
...
# metadata.export_script_to_vault(tbl,'satellite','collect/sat/MODIS/GetMODIS_POC_8day_cl1.py','collect.txt')   