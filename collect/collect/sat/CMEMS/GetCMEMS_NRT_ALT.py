...
def get_rep_uv_sla(yr, mn, dy):
    ...
    c3 = '" --date-max "'
    c4 = (
        '" --variable err --variable vgosa --variable vgos --variable sla --variable adt --variable ugosa --variable ugos --out-dir %s --out-name '
        % cfgv.nrt_alt_raw
    )
    ...
    c5 = (
        cfgv.nrt_alt_prefix
        + str(yr * 1000 + date(yr, mn, dy).timetuple().tm_yday).zfill(3)
        + ".nc"
    )
