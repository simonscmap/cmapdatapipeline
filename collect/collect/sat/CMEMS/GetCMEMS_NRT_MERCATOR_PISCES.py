...
def piscesFileExist(days=get_all_available_days()):
    for dt in days:
        path = (
            cfgv.nrt_mercator_pisces_raw
            + cfgv.nrt_mercator_pisces_prefix
            + str(dl.dateToDayn(dt))
            + ".nc"
        )
        print(path)
        if not os.path.isfile(path):
            print(
                "The following file not found in the opedia vault: "
                + path
                + ", Date: "
                + dt.strftime("%Y-%m-%d")
            )
...
def get_nrt_mercator_pisces_raw(dt):
    ...
    c3 = '" --date-max "'
    c4 = (
        '" --depth-min 0.493 --depth-max 5727.918000000001 --variable PHYC --variable O2 --variable NO3 --variable PO4 --variable Si --variable Fe --variable PP --variable CHL --out-dir %s --out-name '
        % cfgv.nrt_mercator_pisces_raw
    )
...
while dt <= dt2:
    try:
        ...
        print(
            "Dowloading Mercator_Pisces >>>  date: %s,  DayNumber: %s"
