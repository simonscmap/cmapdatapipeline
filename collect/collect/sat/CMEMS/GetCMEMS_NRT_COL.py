...
def Checklist(startDay, endDay):
    for index in range(startDay, endDay + 1):
        dml.updateChecklist(
            index,
            dl.daynToDate(index).year,
            dl.daynToDate(index).month,
            dl.daynToDate(index).day,
            "NRT_COL_Raw",
            1,
        )
    return
...
c1 = 'python2.7 motu-client.py -u {usr} -p {passwd} -m http://cmems.isac.cnr.it/mis-gateway-servlet/Motu -s OCEANCOLOUR_GLO_OPTICS_L4_NRT_OBSERVATIONS_009_083-TDS -d dataset-oc-glo-opt-multi-l4-%s_4km_8days-rt-v01 -x -179.97917175293 -X 179.97917175293 -y -89.97917175293 -Y 89.979164123535 -t "'
c4 = '" %s -o %s -f '
