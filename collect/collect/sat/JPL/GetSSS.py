...
def getURL(yr, dayn):
    base = (
        "https://podaac-opendap.jpl.nasa.gov:443/opendap/allData/smap/L3/RSS/V3/8day_running/SCI/70KM/%d/%s/"
        % (yr, str(dayn).zfill(3))
    )
    ...
    if dayn2 > lastday:
        yr += 1
        dayn2 = dayn2 - lastday
    fname = "RSS_smap_SSS_L3_8day_running_70km_%d_%s_FNL_v03.0.nc?" % (
        yr,
        str(dayn2).zfill(3),
    )
...
def get_nrt_sss(yr, mn, dy):
    ...
    print(
        "Dowloading SSS >>>  date: %s,  DayNumber: %s"
        % (datetime.strftime(dt, "%Y-%m-%d"), index)
    )
